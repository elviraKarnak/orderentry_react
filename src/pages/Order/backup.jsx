...
function EditNewOrder() {
    ...
    const getSingleOrderDataById = async (o_id, c_id) => {
        ...
        if (o_responce.data.status) {
            ...
            for (var i of orderData.order_item_details) {
            ...
            // === replace_OrderItemsData ====
            dispatch({ type: "replace_OrderItemsData", value: itemData });
        ...
        var responce = await orderService.singleOrderItemList(o_id);

        if (responce.data.status) {
            const orderData = responce.data.data;

            // alert(orderData.delivary_date)
            setDeliveryDate(moment(orderData.delivary_date).toDate())
    ...
    // useEffect(() => {
    //     if (DeliveryDate != null && AddItem === true) {
    //         // alert(AddItem)
    //         // console.log("product search run")
    //         ProductSearch()
    //     }
    // }, [ProductDataSearch])
