...
function NewOrder() {
    ...
    const ProductList2 = async () => {
        try {
            ...
            if (!AddItem) {
                ...
                const responce = await axios.get(`https://wordpress-658092-2176352.cloudwaysapps.com/wp-json/getproducts/v1/product_filter_listing?model=landed&date_text=${deliveryDate}&page_no=1&pact=&pcolor&psource&isbybunch&searchquery&filter_opt`);
    ...
    const salePriceSet = (change_price, original_price, p_id, index) => {
    ...
    const NewOrderSave = async () => {
        ...
        for (var item of userState.OrderItemsData) {
            var temp = {
                ...
                sale_price: item.product_details.landed_price,
    ...
    const quantityListValueset = (index, quantity, unit_price, cost_price) => {
        ...
        if (indexToUpdate !== -1) {
            ...
            // ------- total -------
            updatedData[indexToUpdate] = { ...updatedData[indexToUpdate], total: (Number(quantity) * Number(unit_price)).toFixed(2) };
            ...
            <Container>
                ...
                <div className="order-total-table">
                        ...
                        <tbody>
                            {userState.OrderItemsData.map((item, index) => (<>
                                <tr key={index}>
                                    ...
                                    <td>
                                        <img src={item.product_details.image} alt="" style={{ width: "70px", height: "90px" }} />
                                    </td>
                                    <td></td>
                                    <td>{item.product_details.cat}</td>
                                    <td>{item.product_details.color}</td>
                                    <td>{item.product_details.landed_price}</td>
                                    <td>{item.quantity}</td>
                    ...
                    <div className="add-line-item">
                        <img src={AddCircleOut} alt="" /> {" "}

                        <Button onClick={ProductList2}>Add a line item </Button>

                        {" "}{" "}
                        Margin {userState.TotalPM.margin}%  Subtotal ${userState.TotalPM.total}
                    </div>
                            ...
                            <tbody>
                                ...
                                {userState.ProductData.length > 0 && userState.ProductData.map((item, index) => (<>
                                    <tr key={index}>
                                        <td>{item.product_details.name}</td>
                                        <td>
                                            <img src={item.product_details.image} alt="" style={{ width: "70px", height: "90px" }} onClick={() => setImageShowModalUrl(item.product_details.image)} />
                                        </td>
                                        <td>{item.product_details.cat}</td>
                                        <td>{item.product_details.color}</td>
                                        <td></td>
                                        ...
                                        <td>
                                            <input type="number" value={item.product_details.landed_price} onChange={(e) => salePriceSet(e.target.value, item.product_details.landed_price, item.product_details.id, index)} />
                                        </td>
